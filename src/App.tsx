...
function App() {
  ...
  const getFollowersWithAccountUserName = async () => {
    const url = `https://graph.facebook.com/v14.0/${instagramAccount.instagram_business_account.id}?fields=business_discovery.username(${username}){followers_count}&access_token=${userToken}`;
    const response = await fetch(url);
